-- max_daily_return_1m Gold implementation.
-- value = 월중 최대 일별 분할조정 가격수익률 (유효 관측 10개 이상)
...
WITH certified AS (
    ...
    SELECT
        ...
        max_daily_return_1m::double precision AS value,
    ...
    WHERE month_rank = 1
      ...
      AND market_cap > 0
      AND adj_close > 0
      AND max_daily_return_observations_1m >= 10
