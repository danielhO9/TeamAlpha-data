...
def export_universe(monthly_csv, destination):
...
def expected_partitions(conn, manifest, start, end):
    ...
    with conn.cursor() as cur:
        cur.execute('''SELECT p.asset_id,i.identifier,p.trade_date
            FROM price_daily p JOIN asset_identifier i ON p.asset_id=i.asset_id
              AND i.source='KRX' AND i.identifier_type='ticker'
              AND i.valid_from<=p.trade_date AND (i.valid_to IS NULL OR i.valid_to>=p.trade_date)
            JOIN dq_run d ON d.run_id=p.quality_run_id AND d.status='CERTIFIED'
            WHERE p.source='KRX' AND p.asset_id=ANY(%s) AND p.trade_date BETWEEN %s AND %s
              AND p.market IN ('KOSPI','KOSDAQ')
            ORDER BY p.asset_id,p.trade_date''',(ids,start,end))
        rows = cur.fetchall()
    conn.rollback()  # read-only planning transaction
    seen = set(); partitions = {}
    for aid,ticker,day in rows:
        if (aid,day) in seen:
            raise ValueError('ambiguous identifier or duplicated expected date')
        seen.add((aid,day))
        # Short bounded windows allow resumption and avoid per-request truncation.
        key = (aid,ticker,(day-start).days//90)
        partitions.setdefault(key,[]).append(day)
    if not partitions:
        raise ValueError('no certified expected trading dates')
    return partitions
...
def run(*, conn, manifest_uri, policy_uri, root, start, end, publish=False, refresh=False, client=None):
    ...
    for (aid,ticker,_),dates in partitions.items():
        for venue in ('J','NX','UN'):
            vd=dates if venue=='J' else [d for d in dates if d>=NXT_START]
            if not vd:continue
            key=digest({'aid':aid,'ticker':ticker,'dates':[d.isoformat() for d in vd],
                        'venue':venue,'policy':policy,'universe':claimed})
...
def daily(day, *, conn):
    ...
    with conn.cursor() as cur:
        cur.execute("SELECT DISTINCT p.trade_date FROM price_daily p JOIN dq_run d ON d.run_id=p.quality_run_id AND d.status='CERTIFIED' WHERE p.source='KRX' AND p.trade_date<=%s ORDER BY p.trade_date DESC LIMIT 5",(target,))
        dates=[r[0] for r in cur.fetchall()]
    conn.rollback()
    if len(dates)<5:raise RuntimeError('five certified market days required')
...
def main():
    ...
    export=sub.add_parser('export-universe');export.add_argument('--monthly-csv',required=True);export.add_argument('--dest',required=True)
    ...
    if args.command=='export-universe':
        print(json.dumps(export_universe(args.monthly_csv,args.dest)));return
