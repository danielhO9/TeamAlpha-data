...
`KIS_HISTORY_INTERVAL_SECONDS`는 모든 조회 스레드가 공유하는 최소 호출 간격이다.
기본값은 신규 계정에도 보수적인 `1.05`초이며 `1/18`초보다 작은 값은 거부한다.
`KIS_HISTORY_WORKERS`는 동시에 조회·Bronze 보관하는 파티션 수(기본 1, 최대 32)다.
계정별 허용 유량을 확인한 실전 백필에서는 예를 들어 간격 `0.06`, worker `12`를
명시하고 실측 오류율·처리량을 확인한다. 초당 18회는 공급자 상한이지 처리 속도 보장이 아니다.
[한투 유량 공지](https://apiportal.koreainvestment.com/community/10000000-0000-0011-0000-000000000001/post/d0d1a83f-6f8d-4437-9700-6d26702fd989).

단일 Client의 제한기가 재시도를 포함해 호출 간격을 통제한다. `EGW00201`은 전체
스레드의 호출을 잠시 멈추고, `EGW00316`과 연결·시간 초과도 최대 3회 재시도한다.
한도는 계정 단위이므로 같은 계정의 다른 수집 프로세스를 함께 실행하지 않는다.
HTTP 세션은 스레드별로 분리하고 토큰 발급·갱신은 한 번만 수행한다.

조회·S3 저장은 병렬로 수행하지만 커버리지·수급·시장 범위 검증은 동일하다.
RDS 게시와 CERTIFIED 처리, 체크포인트 기록은 호출 스레드에서 순서대로 실행한다.
메모리에 대기하는 결과는 worker 수로 제한한다. 속도를 바꿔도 정책·manifest·요청 기간을
유지하면 체크포인트 키가 같아서 이미 완료한 파티션은 재조회·재적재하지 않는다.
S3 체크포인트는 페이지 단위 목록 조회로 한 번에 확인한다. 실패했거나 중단되어
체크포인트가 없는 파티션만 다시 수집한다. 속도 변경을 위해 `--refresh`를 사용하지 않는다.

## 로그인 없는 한투 일일 운영

사용자가 한투의 시장 구분을 신뢰하기로 선택한 경우 `KIS_DAILY_MODE=PROVIDER_TRUST`와
`short_market_basis=PROVIDER_TRUST` 정책을 함께 사용한다. 정책에는 사용자 수락 근거 URI가
필수다. 기존 KRX 외부 대조 근거와 마지막 대조 날짜는 바꾸지 않고, 새 공매도 관측에
`market_scope_basis=PROVIDER_TRUST`를 명시한다. 미래 날짜를 KRX 대조 완료로 표시하지 않는다.
원거래량을 분모로 하는 비율, 수량·금액·통합 수급 대조, 날짜·식별자 검사는 동일하게 적용한다.

`KIS_UNIVERSE_URI`는 승인된 역사적 연구 종목 합집합을 고정한 입력 버전이다.
이 모드에서는 오래된 `as_of`를 오늘로 바꾸거나 새로운 연구 편입을 추측하지 않는다.
연구 패널을 새로 발행해 감시 종목을 바꿀 때는 해당 manifest와 일치하는 reference seed도
갱신해야 한다. 기존 source hash가 달라지면 자동 실행은 중단한다.

`KIS_DAILY_REFERENCE_URI`는 검증된 NXT 출범/편출입 상태, 종목별 상장기간, 기존 NXT 구간을
담은 체크포인트다. 인증 가격의 예상 날짜가 비면 한투 주식기본조회 `lstg_abol_dt`로 실제 상장폐지를
확인하고, NXT의 일별 공개 자료로 거래 가능 구간을 갱신한다. 가격 부재만으로 폐지 처리하지 않는다. 브라우저 로그인이나 LLM이 필요 없다. 누락된 NXT 행을 곧바로 거래 불가나
0으로 해석하지 않는다. 기존 공식 편출입 상태와 영구 제외 근거로 설명되지 않으면 실패한다.
상장기간 변경 후 독립적인 거래일 기대치와 CERTIFIED 가격·PIT 식별자를 대조하고,
기간 밖 가격이나 새 식별자는 자동으로 제외하지 않고 오류로 처리한다.

최근 5거래일을 재조회하며, 중단 기간이 더 길면 마지막 성공 체크포인트 이후 날짜도 포함한다.
40일 넘게 중단된 경우 명시적 reference catch-up이 필요하다. reference 체크포인트는
모든 파티션 게시와 마지막 RDS 키 집합 대조가 성공한 뒤에만 전진한다. 원본과 근거는
내용 해시로 S3에 보관하며, 기대 키와 실제 키의 누락·초과·중복 및 비율 상태를 최종 확인한다.
기존 일일 인증 잠금 안에서 실행해 계좌/API 호출과 RDS 게시 작업이 중첩되지 않게 한다.

운영 활성화 변수: `KIS_FLOWS_ENABLED=1`, `KIS_DAILY_MODE=PROVIDER_TRUST`,
`KIS_DAILY_REFERENCE_URI`, 기존 `KIS_UNIVERSE_URI`, `KIS_POLICY_URI`, `KIS_BRONZE_ROOT`.
