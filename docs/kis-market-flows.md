...
운영 사전 점검에서 현재 연구 종목의 asset.listed_from이 모두 비어 있는 것이 확인됐다. 독립적인 예상 날짜 검사는 이 경우 실패하도록 되어 있으므로, 실제 전체 백필 전에 검증된 상장·폐지 메타데이터를 준비해야 한다. NXT 대조 완료를 가격·수급 전체 적재 완료로 해석하지 않는다.

## 상장·폐지 메타데이터 후보 (2026-09-12)
...
- 오상헬스케어(036220)와 우양에이치씨(101970)는 실제 폐지 후 재상장한 이력이 있어 `periods`로 구간을 나눈다. 단일 listed_from/listed_to로 평탄화하면 중간 비상장 기간에 허위 결측이 생기므로 `REVIEW_REQUIRED`를 유지한다.

2,814종목은 `PENDING_RDS_VALIDATION`, 위 2종목은 `REVIEW_REQUIRED` 상태다. 전체 후보 구간과 2015년 이후 실제 연구 편입 월별 기록 사이의 구간 이탈은 0건이었다. 이는 RDS 일별 가격 대조나 종목 동일성의 최종 인증을 대신하지 않는다. 다음 반영 단계는 RDS 가격·식별자 대조, 재상장 구간을 사용하는 예상 날짜 계약 연결, 품질 실행과 함께 메타데이터 발행 순서다. SSO 만료로 이번 준비 작업에서 RDS/S3 반영이나 스케줄 활성화는 하지 않았다.
