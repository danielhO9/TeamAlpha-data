...
유니버스: DART corpCode.xml 에서 stock_code 가 있는 상장사(상폐 포함) 전체 — pykrx 불필요.
...
사용:
  ...
  uv run python -m bronze.financials --from 2015 --to 2026 --dest s3
"""
...
def load_listed_corps() -> list[tuple[str, str]]:
    """corpCode.xml 다운로드 → [(corp_code, stock_code)] (상장사=stock_code 있음, 상폐 포함).
    인자는 API 키 하나뿐. 응답은 ZIP(CORPCODE.xml 1개)."""
                ...
                for batch in batches:
                    ...
                    for tkr, rows in by_ticker.items():
                        ...
                        write_text(json.dumps(rows, ensure_ascii=False),
                                   f"{base}/financials/dart/year={year}/corp={tkr}/{reprt}.json")
                        saved += 1
                    time.sleep(CALL_GAP_SEC)
    except QuotaExceeded as exc:
        print(f"[financials] 사용한도 초과로 중단: {exc} — 저장 {saved} / 스킵 {skipped} / 무데이터 {nodata}")
        print("[financials] 내일 같은 명령으로 재개하면 이어서 받음.")
        return

    print(f"[financials] 완료: 저장 {saved} / 스킵 {skipped} / 무데이터 {nodata}")
